...
describe('Search Functions', () => {
  ...
  describe('searchComuni', () => {
    it('dovrebbe trovare comuni per nome esatto', () => {
      const risultati = searchComuni(mockComuni, 'Milano', { exactMatch: true })
    ...
    it('dovrebbe verificare tutte le tipologie di provincia', () => {
        ...
        const capoluoghi = searchComuni(mockComuni, 'a', { soloCapoluoghi: true })
    ...
    it('dovrebbe verificare tipologie di regione', () => {
      ...
      expect(risultatiLombardia[0].item.tipologiaRegione).toBe('statuto ordinario')
    })
    ...
    it('dovrebbe essere case-sensitive quando richiesto', () => {
        const risultatiCorretto = searchComuni(mockComuni, 'Milano', { caseSensitive: true })
        expect(risultatiCorretto.length).toBeGreaterThan(0)
        
        // E testa che case sbagliato non funziona
        const risultatiSbagliato = searchComuni(mockComuni, 'milano', { caseSensitive: true })
  ...
  describe('searchRegioni', () => {
    it('dovrebbe trovare regioni per nome esatto', () => {
      const risultati = searchRegioni(mockRegioni, 'Lombardia', { exactMatch: true })
  ...
  describe('getCapoluoghi', () => {
    ...
    it('dovrebbe escludere non capoluoghi', () => {
      const capoluoghi = getCapoluoghi(mockComuni)
      const nonCapoluoghi = capoluoghi.filter(c => c.nome === 'San Giuliano Milanese')
  ...
  describe('getProvinceByRegione', () => {
    it('dovrebbe trovare province per regione usando comuni', () => {
      const province = getProvinceByRegione(mockProvince, mockComuni, 'Lombardia')
    ...
    it('dovrebbe restituire array vuoto per regione inesistente', () => {
      const province = getProvinceByRegione(mockProvince, mockComuni, 'RegioneFantastica')
  ...
  describe('getProvinceByRegioneWithRegioni', () => {
    it('dovrebbe trovare province per regione usando regioni', () => {
      const province = getProvinceByRegioneWithRegioni(mockProvince, mockRegioni, 'Lombardia')
    ...
    it('dovrebbe ordinare alfabeticamente', () => {
      ...
      for (let i = 1; i < province.length; i++) {
        expect(province[i-1].nome.localeCompare(province[i].nome)).toBeLessThanOrEqual(0)
      }
    })

    it('dovrebbe gestire stringhe con spazi', () => {
      const province1 = getProvinceByRegioneWithRegioni(mockProvince, mockRegioni, ' Lombardia ')
      const province2 = getProvinceByRegioneWithRegioni(mockProvince, mockRegioni, 'Lombardia')
